...
import type { LetterVortexConfig, DeepPartial, EasingCurve } from '@/animations/letter-vortex';
      ...
      <ScrollArea className="flex-1 min-h-0">
        <div className="flex flex-col gap-6 p-4">
          ...
          <Section title="Word Settings">
            <Field label="Target Word" tooltip="The word that will be assembled from small letter pixels (dot-matrix style).">
              <Input
                value={config.word.target}
                onChange={(e) => onChange({ word: { target: e.target.value.toUpperCase() } })}
              />
            </Field>
            <Field label="Font Family" tooltip="Font used to render all letters.">
              <Select
            ...
            <SliderField
              label="Target Font Size" value={config.word.targetFontSize} min={100} max={600} step={10}
              tooltip="Font size used for Canvas2D pixel sampling. Larger = more detail/slots."
              onChange={(v) => onChange({ word: { targetFontSize: v } })}
            />
            <SliderField
              label="Letter Height" value={config.word.letterHeight} min={0.3} max={3.0} step={0.05}
              tooltip="Y-axis multiplier for assembled word height in 3D space."
              onChange={(v) => onChange({ word: { letterHeight: v } })}
            />
            <SliderField
              label="Letter Spacing" value={config.word.letterSpacing} min={20} max={200} step={1}
            ...
            <Field label="Color" tooltip="Color of all letters.">
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => onChange({ word: { color: e.target.value } })}
                  className="h-8 w-10 cursor-pointer rounded border border-border bg-transparent p-0.5"
                />
                <span className="text-xs text-muted-foreground">{config.word.color}</span>
              </div>
            </Field>
          </Section>
          ...
          <Section title="Particles">
            <Field label="Source Characters" tooltip="Pool of characters used for the flying letter-pixels.">
              <Input
                value={config.particles.sourceChars}
                onChange={(e) => onChange({ particles: { sourceChars: e.target.value.toUpperCase() } })}
              />
            </Field>
            <SliderField
              label="Orbit Count" value={config.particles.orbitCount} min={50} max={2000} step={10}
            ...
            />
            <SliderField
              label="Grid Resolution" value={config.particles.gridResolution} min={4} max={24} step={1}
            ...
            />
            <SliderField
              label="Orbit Min Size" value={config.particles.orbitMinSize} min={2} max={40} step={1}
              tooltip="Smallest sprite size when far from camera (orbit phase)."
              onChange={(v) => onChange({ particles: { orbitMinSize: v } })}
            />
            <SliderField
              label="Orbit Max Size" value={config.particles.orbitMaxSize} min={10} max={120} step={1}
              tooltip="Largest sprite size when close to camera (orbit phase)."
              onChange={(v) => onChange({ particles: { orbitMaxSize: v } })}
            />
            <SliderField
              label="Assembled Size" value={config.particles.assembledSize} min={4} max={40} step={1}
              tooltip="Fixed sprite size when locked into a dot-matrix grid slot."
              onChange={(v) => onChange({ particles: { assembledSize: v } })}
            />
            <SliderField
              label="Min Opacity" value={config.particles.minOpacity} min={0} max={1} step={0.01}
              tooltip="Opacity range based on depth — far letters are more transparent."
              onChange={(v) => onChange({ particles: { minOpacity: v } })}
            />
            <SliderField
              label="Max Opacity" value={config.particles.maxOpacity} min={0} max={1} step={0.01}
              tooltip="Maximum opacity for letters closest to the camera."
              onChange={(v) => onChange({ particles: { maxOpacity: v } })}
            />
            <Field label="Depth Scale" tooltip="Toggle whether size and opacity change based on Z position during orbit.">
              <button
                type="button"
                onClick={() => onChange({ particles: { depthScale: !config.particles.depthScale } })}
              ...
              >
                <span className={`pointer-events-none block h-5 w-5 rounded-full bg-background shadow-lg ring-0 transition-transform ${config.particles.depthScale ? 'translate-x-5' : 'translate-x-0'}`} />
              </button>
            </Field>
          </Section>
          ...
          <Section title="Animation Phases">
            <SliderField
              label="Cycle Duration" value={config.animation.cycleDuration} min={2000} max={30000} step={500}
            ...
            {/* Scatter */}
            <SliderField
              label="Scatter Duration" value={config.animation.phases.scatter.duration} min={500} max={10000} step={100}
              tooltip="Time letters spend flying freely in orbit."
              format={(v) => `${(v / 1000).toFixed(1)}s`}
              onChange={(v) => onChange({ animation: { phases: { scatter: { duration: v } } } })}
            />
            <Field label="Scatter Curve" tooltip="Easing function for scatter phase movement.">
              <Select
                value={config.animation.phases.scatter.speedCurve}
                onValueChange={(v) => onChange({ animation: { phases: { scatter: { speedCurve: v as EasingCurve } } } })}
              >
                <SelectTrigger className="w-full"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            {/* Assemble */}
            <SliderField
              label="Assemble Duration" value={config.animation.phases.assemble.duration} min={500} max={10000} step={100}
              tooltip="Time letters take to fly into their target positions."
              format={(v) => `${(v / 1000).toFixed(1)}s`}
              onChange={(v) => onChange({ animation: { phases: { assemble: { duration: v } } } })}
            />
            <Field label="Assemble Curve" tooltip="Easing function controlling the fly-in motion.">
              <Select
                value={config.animation.phases.assemble.speedCurve}
                onValueChange={(v) => onChange({ animation: { phases: { assemble: { speedCurve: v as EasingCurve } } } })}
              >
                <SelectTrigger className="w-full"><SelectValue /></SelectTrigger>
                <SelectContent>
                  {EASING_OPTIONS.map((o) => (
            ...
            <SliderField
              label="Stagger Delay" value={config.animation.phases.assemble.staggerDelay} min={0} max={200} step={5}
              tooltip="Delay between each letter starting its fly-in, in milliseconds."
              format={(v) => `${v}ms`}
              onChange={(v) => onChange({ animation: { phases: { assemble: { staggerDelay: v } } } })}
            />

            {/* Hold */}
            <SliderField
              label="Hold Duration" value={config.animation.phases.hold.duration} min={200} max={5000} step={100}
              tooltip="How long the assembled word is displayed before dissolving."
              format={(v) => `${(v / 1000).toFixed(1)}s`}
              onChange={(v) => onChange({ animation: { phases: { hold: { duration: v } } } })}
            />

            {/* Dissolve */}
            <SliderField
              label="Dissolve Duration" value={config.animation.phases.dissolve.duration} min={200} max={5000} step={100}
              tooltip="Time letters take to break apart and return to orbit."
              format={(v) => `${(v / 1000).toFixed(1)}s`}
              onChange={(v) => onChange({ animation: { phases: { dissolve: { duration: v } } } })}
            />
            <Field label="Dissolve Curve" tooltip="Easing function for the dissolve phase.">
              <Select
                value={config.animation.phases.dissolve.speedCurve}
                onValueChange={(v) => onChange({ animation: { phases: { dissolve: { speedCurve: v as EasingCurve } } } })}
              >
                <SelectTrigger className="w-full"><SelectValue /></SelectTrigger>
                <SelectContent>
                  {EASING_OPTIONS.map((o) => (
          ...
          <Section title="Export">
            <Field label="Quality Preset" tooltip="Resolution and bitrate of exported video.">
              <Select
                value={config.export.defaultQuality}
                onValueChange={(v) => onChange({ export: { defaultQuality: v as LetterVortexConfig['export']['defaultQuality'] } })}
              >
                <SelectTrigger className="w-full"><SelectValue /></SelectTrigger>
                <SelectContent>
                  {(Object.keys(config.export.qualityPresets) as Array<keyof typeof config.export.qualityPresets>).map((key) => {
                    const p = config.export.qualityPresets[key];
                    return (
                      <SelectItem key={key} value={key}>{key} ({p.width}×{p.height})</SelectItem>
                    );
                  })}
                </SelectContent>
              </Select>
            ...
            <SliderField
              label="Duration" value={config.export.duration} min={1} max={60} step={1}
              tooltip="How many seconds to export."
              format={(v) => `${v}s`}
              onChange={(v) => onChange({ export: { duration: v } })}
            />
            <Field label="FPS" tooltip="Frames per second for the exported video.">
              <Select
            ...
            <Field label="Format" tooltip="WebM (browser-native) or MP4.">
              <Select
                ...
                onValueChange={(v) => onChange({ export: { format: v as LetterVortexConfig['export']['format'] } })}
              >
                <SelectTrigger className="w-full"><SelectValue /></SelectTrigger>
                <SelectContent>
